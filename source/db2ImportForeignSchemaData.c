...
bool isForeignSchema(DB2Session* session, char* schema) {
  ...
  db2Debug1("> %s::isForeignSchema(schema: '%s')", __FILE__, schema);
  ...
  fResult = (count > 0);
   db2Debug1("< %s::isForeignSchema : %s, result = %s", __FILE__, schema, fResult ? "true" : "false");
 return fResult;
}
...
char** getForeignTableList(DB2Session* session, char* schema, int list_type, char* table_list){
  ...
  int         tabidx        = 0;
  char**      tabnames      = NULL;
   db2Debug1("> %s::getForeignTableList(schema: '%s', list_type: %d, table_list: '%s')", __FILE__, schema, list_type, table_list);
  switch(list_type){
  ...
  db2Debug1("< %s::getForeignTableList : [%d]", __FILE__, tabidx-1);
  return tabnames;
}
...
DB2Table* describeForeignTable (DB2Session* session, char* schema, char* tabname) {
  ...
  db2Debug1("> %s::db2DescribeForeignTable(schema: %s, tablename: %s)", __FILE__, schema, tabname);
  ...
  db2Debug1("< %s::db2DescribeForeignTable - returns: %x", __FILE__, reply);
  return reply;
}
...
static void describeForeignColumns(DB2Session* session, char* schema, char* tabname, DB2Table* db2Table) {
  ...
  db2Debug1("> %s::describeForeignColumn(schema: %s, tabname: %s)", __FILE__, schema, tabname);
  ...
  /* release the statement handle */
  db2FreeStmtHdl(stmtp, session->connp);
  db2Debug1("< %s::describeForeignColumn", __FILE__);
}