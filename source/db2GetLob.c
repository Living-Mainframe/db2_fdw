...
void db2GetLob (DB2Session* session, DB2ResultColumn* column, char** value, long* value_len, unsigned long trunc) {
  ...
  int            extend = 0;
  db2Entry(1,"> db2GetLob.c::db2GetLob");
  db2Debug(2,"column->colName: '%s'",column->colName);
  db2Debug(2,"column->resnum :  %d ",column->resnum);
  /* initialize result buffer length */
  *value_len = 0;
  /* read the LOB in chunks */
  do {
    db2Debug(2,"value_len: %ld",*value_len);
    db2Debug(2,"reading %d byte chunck of data",sizeof(buf));
    ...
    if (rc != 100) {
      switch(ind) {
        case SQL_NULL_DATA:
          db2Debug(3,"data length is null (SQL_NULL_DATA)");
          extend = 0;
        break;
        case SQL_NO_TOTAL:
          db2Debug(3,"undefined data length (SQL_NO_TOTAL)");
          extend = LOB_CHUNK_SIZE;
        break;
        default:
          db2Debug(3,"bytes still remaining: %d", ind);
          extend = (ind < LOB_CHUNK_SIZE) ? ind : LOB_CHUNK_SIZE;
        break;
      }
      /* extend result buffer by ind */
      db2Debug(2,"value_len: %ld", *value_len);
      db2Debug(2,"extend   : %d", extend);
      if (*value_len == 0) {
        if (extend > 0) {
          *value = db2alloc ("lob_value", *value_len + extend + 1);
        } else {
          *value = NULL;
          db2Debug(3,"not allocating space since the LOB value is apparently NULL");
        }
      } else {
      ...
      db2Debug(2,"*value    : %x", *value);
      db2Debug(2,"*value_len: %x", *value_len);
      if (*value != NULL) {
        db2Debug(3,"memcpy(%x,%x,%d)",*value+*value_len,buf,extend);
        memcpy(*value + *value_len, buf, extend);
        /* update LOB length */
        *value_len += extend;
      } else {
        db2Debug(3,"skipping value copy, since value is NULL");
      }
    }
  ...
  if (*value != NULL) {
    (*value)[*value_len] = '\0';
    db2Debug(2,"strlen of lob: %ld", strlen(*value));
  } else {
    db2Debug(2,"strlen of lob: 0 since *value is NULL");
  }
  db2Exit(1,"< db2GetLob.c::db2GetLob");
}
