  ...
  if (expr != NULL) {
    ...
    switch (expr->type) {
      ...
      case T_NullTest: {
        ...
        arg = deparseExpr(ctx->root, ctx->foreignrel, ((NullTest*) expr)->arg, ctx->params_list);
        db2Debug2("  T_NullTest arg: %s", arg);
        if (arg != NULL) {
          appendStringInfo (ctx->buf, "(%s IS %sNULL)", arg, ((NullTest*)expr)->nulltesttype == IS_NOT_NULL ? "NOT " : "");
        }
        db2free(arg);
      }
      break;
